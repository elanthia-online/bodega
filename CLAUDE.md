...
- `docs/data/towns.json` - Town manifest generated by processor.rb; the web
  app and `fetch-site-data` discover towns from it (no hardcoded town lists,
...
1. **Scrape** — `automation/bin/fetch-site-data` restores the previous
   processed town files (discovered via the site's `towns.json` manifest) +
   tracking (`added_items`/`removed_items`) + `state.json` from the live
   site into `docs/data/`, then runs the scan. Auto mode does one **full** scan per UTC day (the
